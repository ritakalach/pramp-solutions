...
def flat_dict_recurse(nested_dict, flat_dict, key_path=""):
  for key, value in nested_dict.items():
    is_key = int(bool(key)) # 1 if key isn't empty, else 0
    # Case 1: not a nested dict, so we csn add items to final output 
    if not isinstance(value, dict): # the value is of a primitive type
      if not key_path:
        flat_dict[key] = value
      else:
